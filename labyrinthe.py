...
COTE_CASE = 40 #nombre maximale de coté du labyrinthe

class Cellule: #initie une case
    def __init__(self):
        self.mur_bas = True  # True signifie que le mur est fermÃ©
        self.mur_droit = True # False signifie que le mur est ouvert
        

class Labyrinthe: #initie le labyrinthe en donnant le nombre de coté en largeur et en longueur
    def __init__(self, largeur, hauteur):
        self.hauteur = hauteur
        self.largeur = largeur
        self.grille = [[Cellule() for j in range(largeur)] for i in range(hauteur)]
        self.generation()#crée le labyrinthe

    def solution(self, depart_ligne, depart_colonne, arrivee_ligne, arrivee_colonne):
        """
        Renvoie la liste de directions Ã  suivre pour se rendre de la cellule
        (depart_ligne, depart_colonne) Ã  la cellule (arrivee_ligne, arrivee_colonne).
        Exemple de retour : ['n', 'e', 'e', 's', 'o']

        """
        list_solution = []
        if mur_bas == False :
            list_solution.append(indice_mur)
        print(list_solution)
    ...
    def generation(self): #crée le labyrinthe
#         print(self.largeur,self.hauteur)
        nombre_cellules = self.largeur*self.hauteur #largeur et longueur du labyrinthe (nombre total de cellules)
        zones_cellules = list(range(nombre_cellules))
        print(zones_cellules)
        print("------------")
        # Les cellules sont numérotées en ligne, avec les numéros 0 Ã  
        # largeur-1 sur la premiÃ¨re ligne, largeur Ã  2*largeur-1 sur la
        # deuxiÃ¨me ... jusqu'Ã  hauteur*largeur-1 en bas Ã  droite
        
        murs_modifiables = list(range(2*nombre_cellules-self.largeur-self.hauteur)) #correspond au nombre de mur internes du labyrinthe
        print(murs_modifiables)
        # Tous les murs bas et droits sont modifiables sont ceux en bas ou Ã
        # droite du labyrinthe. Ils sont numÃ©rotÃ©s en commençant par les murs
        ...
        # ceux allant de self.largeur*self.hauteur-self.hauteur Ã  
        # 2*self.largeur*self.hauteur-self.largeur-self.hauteur-1 les horizontaux.
        
        indice_premier_non_modifiable = len(murs_modifiables) #dernière cellule du labyrinthe
        print("------------")
        print(indice_premier_non_modifiable) 
        # Lorsqu'un mur est choisi, on le met Ã  la fin du tableau pour ne
        # pas le rechoisir. Ce serait plus simple de le supprimer de la liste
        # mais plus couteux car la suppression est en O(n).
        
        modifications = 0 #nombre de modification apporter au mur d'une cellule
#         print(modifications)
        while modifications < nombre_cellules - 1:
            #traite les murs qui doivent être modifiés l'ordre est : mur bas et ensuite mur droit,
            #le premier mur est d'indice 0 donc on commence à compter à partir de 0
            ...
            murs_modifiables[indice_mur], murs_modifiables[indice_premier_non_modifiable] = murs_modifiables[indice_premier_non_modifiable], murs_modifiables[indice_mur]
            print("mur",indice_mur,"a été modifié : ",modifications,"modification")
            
    def fusionner_zones(zones_cellules, indice_cellule1, indice_cellule2):
       zone1 = zones_cellules[indice_cellule1]
       print("''''''''''''''''''''")
       print(zones_cellules[indice_cellule1])
       zone2 = zones_cellules[indice_cellule2]
       print(zones_cellules[indice_cellule2])
    ...
    def modifier_mur(self, mur_candidat, zones_cellules):
        # Le mur sÃ©pare cellule1 et cellule2
        ...
        if zones_cellules[indice_cellule1] != zones_cellules[indice_cellule2]:  
            ...
            return True
        else:   # les deux cellules sont dÃ©jÃ  dans la mÃªme zone, on ne supprime pas le mur
            return False
...
class GfxHMI(tk.Tk):
    def __init__(self, largeur, hauteur):
        ...
        tk.Button(self, text='Tracer', command=self.souris_tresor).pack(side=tk.LEFT, padx=5, pady=5)
        ...
        self.trace_du_chemin = None  # conserve la variable tracÃ© pour pouvoir effacer le chemin
       
    def dessine_cellule(self, i, j, cellule): #trace les murs du labyrinthe
